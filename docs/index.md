...
- **[Annotations](adopter/annotation/index.md)** -- Compile-time annotations (`@MessageDef`,
  `@TemplateDef`) for declaring messages and templates directly in source code.

- **[ASM](adopter/annotation/asm.md)** -- Bytecode-level scanning of message annotations from
  compiled class files using the ASM library -- no class loading required.

- **[Log4j](add-on/log4j/index.md)** -- Apache Log4j 2 integration -- use message-format
  syntax directly in log statements.

- **[Spring](add-on/spring/index.md)** -- Spring Framework integration with `MessageSource`
  adapter, classpath scanning via Spring ASM, and a SpEL parameter formatter.
