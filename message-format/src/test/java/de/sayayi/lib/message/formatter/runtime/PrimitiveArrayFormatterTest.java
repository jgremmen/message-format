 ...
 * @since 0.8.0
 */
public class PrimitiveArrayFormatterTest extends AbstractFormatterTest
{
  @Test
  void testFormattableTypes()
  {
    assertFormatterForType(new PrimitiveArrayFormatter(), boolean[].class);
    assertFormatterForType(new PrimitiveArrayFormatter(), short[].class);
    assertFormatterForType(new PrimitiveArrayFormatter(), int[].class);
    assertFormatterForType(new PrimitiveArrayFormatter(), long[].class);
    assertFormatterForType(new PrimitiveArrayFormatter(), float[].class);
    assertFormatterForType(new PrimitiveArrayFormatter(), double[].class);
  }
  ...
  {
    val formatterService = createFormatterService(new PrimitiveArrayFormatter());
    val messageAccessor = MessageSupportFactory.create(formatterService, NO_CACHE_INSTANCE)
        .setLocale("de-DE")
        .getMessageAccessor();
    ...
    formatterService.addFormatter(new NamedParameterFormatter() {
      ...
      @Override
      public @NotNull Set<FormattableType> getFormattableTypes() {
        return singleton(new FormattableType(int.class, DEFAULT_ORDER));
      }
    });
  ...
  void testEmptyOrNullArray()
  {
    val messageSupport = MessageSupportFactory.create(
        createFormatterService(new PrimitiveArrayFormatter()), NO_CACHE_INSTANCE);
