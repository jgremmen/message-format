...
import de.sayayi.lib.message.internal.part.config.EmptyPartConfig;
...
/**
 * This class represents a message part configuration map.
 *
...
{
  /**
   * An empty part configuration map.
   *
   * @since 0.21.0
   */
  PartConfig EMPTY_CONFIG = EmptyPartConfig.INSTANCE;


  /**
   * Tells whether the configuration contains any values.
   *
   * @return  {@code true} if the configuration contains at least 1 value, {@code false} otherwise
   */
  @Contract(pure = true)
  boolean isEmpty();
  ...
  /**
   * Tells whether the configuration map contains a message entry with the given {@code keyType}.
   *
  ...
  /**
   * Returns the default value from the configuration map.
   ...
   * @since 0.21.0
   */
  @NotNull PartConfig excludeConfigByName(@NotNull Set<String> configNames);
}
